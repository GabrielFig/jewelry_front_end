...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <h1 className="font-serif text-3xl font-bold text-ink mb-3">{t.cart.empty}</h1>
        <p className="text-ink/50 mb-8">{t.cart.emptyDesc}</p>
        <Link href="/products" className="inline-flex items-center justify-center rounded bg-gold px-7 py-3 text-base font-medium text-white hover:bg-gold-dark transition-colors">
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 sm:px-6 py-12">
      <h1 className="font-serif text-4xl font-bold text-ink mb-8">{t.cart.title}</h1>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="rounded-xl border border-ink/10 bg-white p-6 h-fit space-y-4">
          <h2 className="font-serif text-xl font-semibold text-ink">{t.cart.orderSummary}</h2>
