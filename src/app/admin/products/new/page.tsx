...
export default function NewProductPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    let attributes = {};
    if (data.attributes) {
      try { attributes = JSON.parse(data.attributes); } catch {
        setError(t.admin.attributesJson);
        setLoading(false);
        return;
      }
    }
    try {
      ...
      setError(msg ?? t.admin.failedCreateProduct);
    } finally { setLoading(false); }
  };

  return (
    <div className="max-w-2xl">
      <h1 className="font-serif text-3xl font-bold text-ink mb-8">{t.admin.newProductTitle}</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <Input label="SKU" placeholder="RING-GOLD-001" error={errors.sku?.message} {...register("sku")} />
          <Input label={t.admin.price} type="number" step="0.01" error={errors.price_amount?.message} {...register("price_amount")} />
        </div>
        <Input label={t.admin.name} error={errors.name?.message} {...register("name")} />
        <div>
          <label className="text-sm font-medium text-ink/80">{t.admin.descriptionLabel}</label>
        ...
        <div>
          <label className="text-sm font-medium text-ink/80">{t.admin.categoryLabel}</label>
        ...
        <div>
          <label className="text-sm font-medium text-ink/80">{t.admin.attributesLabel}</label>
        ...
        <Input label={t.admin.imageUrl} {...register("image_url")} />
        {error && <p className="text-sm text-red-500">{error}</p>}
        <div className="flex gap-3 pt-2">
          <Button type="submit" loading={loading}>{t.admin.createProduct}</Button>
          <Button type="button" variant="secondary" onClick={() => router.back()}>{t.admin.cancelBtn}</Button>
        </div>
      </form>
    </div>
  );
