    ...
    <div className="min-h-[calc(100vh-10rem)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="font-serif text-3xl font-bold text-ink">{t.auth.welcomeBack}</h1>
          <p className="text-ink/50 mt-2 text-sm">{t.auth.signInSubtitle}</p>
        </div>
        <div className="bg-white rounded-2xl border border-ink/10 shadow-sm p-8">
          ...
          <p className="mt-6 text-center text-sm text-ink/50">
            {t.auth.noAccount}{" "}
            <Link href="/auth/register" className="text-gold hover:underline font-medium">{t.auth.createOne}</Link>
          </p>
        </div>
      </div>
    </div>
